...
/*
    private function getJSONContent(array $content): array
    {
        return array_reduce(
            $content,
            function ($result, $contentItem) {
                ...
                if (isset($contentItem["output"])) {
                    $result[] = $itemLevelShift .
                                "{$contentItem['fileKey']}: ";
                    $result[] = "{" .
                                "\n" . implode($this->getJSONContent($contentItem["output"])) .
                                $itemLevelShift .
                                "}\n";
                } else {
                    $result[] = $itemLevelShift .
    ...
    private function getJSONDifference(array $content): array
    {
        return array_reduce(
            $content,
            function ($result, $contentItem) {
                ...
                if ($firstContentIsArray) {
                    $styledArray = $this->getJSONList(
                        contentItem: $contentItem,
                        currentItemList: $this->getJSONDifference($contentItem["output"]),
                    ...
                    $styledItem = $this->getJSONItem(
                        contentItem: $contentItem,
                        currentShift: $itemLevelShift,
